...
const Profile = () => {
            ...
            text:
              education.length > 0
                ? education
                    .map(
                      (e) =>
                        ` ${e.degree || "N/A"} ${e.major || "N/A"} at ${e.university || "N/A"}, ${
                          e.date || "N/A"
                        }, `
                    )
                    .join("\n")
          ...
          },
          {
            ...
            text:
              recommendations.length > 0
                ? recommendations
                    .map((r) => `${r.name} (${r.relationship}): ${r.recommendation}`)
                    .join(", ")
                : "No information provided."
          }
        ]);
  ...
  const handleProfilePhotoPress = async () => {
    ...
    if (!result.canceled) {
      ...
      try {
        let response = await axios.post("https://tranquil-ocean-74659.herokuapp.com/upload", formData, {
          headers: {
